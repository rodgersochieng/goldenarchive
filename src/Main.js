...
const MusicMixApp = () => {
  ...
  useEffect(() => {
    const targetDownloads = 1243;
    const targetRating = 4.7;
    const targetRatingCount = 86;
    const duration = 5000; // 1.5 seconds
    const steps = 50;
    const increment = targetDownloads / steps;
    const ratingIncrement = targetRating / steps;
    const ratingCountIncrement = targetRatingCount / steps;
    ...
    const timer = setInterval(() => {
      ...
      currentRatingCount += ratingCountIncrement;
      setDownloadCount(Math.min(Math.floor(current), targetDownloads));
      setRatingValue(parseFloat(currentRating.toFixed(1)));
      setRatingCount(Math.min(Math.floor(currentRatingCount), targetRatingCount));
      
      if (current >= targetDownloads) {
        clearInterval(timer);
        setDownloadCount(targetDownloads);
        setRatingValue(targetRating);
        setRatingCount(targetRatingCount);
      }
    }, duration / steps);
  ...
  const validateTransactionCode = () => {
    ...
    // Track payment attempt in Google Analytics
    if (window.gtag) {
  ...
  const handleDownloadClick = (mix) => {
    ...
    // Track download attempt in Google Analytics
    if (window.gtag) {
...
>
  <div className="max-w-4xl mx-auto text-center">
    ...
    <p className={`text-sm sm:text-base mb-10 max-w-md mx-auto leading-relaxed 
    ...
    <div className="flex flex-col sm:flex-row justify-center items-center gap-6 sm:gap-12">
      ...
      {/* Divider dot - purple */}
      <div className={`w-2 h-2 rounded-full ${isDarkMode ? 'bg-purple-500' : 'bg-purple-400'}`}></div>
      ...
      >
        <div className="flex items-center justify-center space-x-2">
          ...
          <p className="text-4xl sm:text-5xl font-bold text-yellow-400">
            <CountUp end={ratingValue} duration={1.5} decimals={1} />
          </p>
        </div>
        <p className={`mt-2 text-sm tracking-wide 
          ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
          Rated by <span className="font-medium text-purple-400">{ratingCount}+</span> fans
        </p>
      </motion.div>
    </div>
  </div>
              ...
              <div className={`p-4 rounded-lg mb-6 ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <h4 className={`font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>How to download</h4>
                    <ol className="list-decimal list-inside mt-2 space-y-1 text-sm">
                      <li>Send Ksh 100 to <strong>Pochi: 254729942447</strong></li>
                      <li>Enter your M-Pesa transaction code below</li>
                      <li>Click "Verify Payment"</li>
                    </ol>
                  </div>
      ...
      {/* Enhanced Footer */}
      <footer className={`pt-12 pb-8 px-6 border-t ${isDarkMode ? 'bg-gray-900 border-gray-800' : 'bg-gray-100 border-gray-300'}`}>
  <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10 text-sm">
    ...
    >
      <h3 className="text-lg font-semibold mb-3">Golden Archive</h3>
      <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
        Curated music mixes blending creativity, culture, and sound design.
        Every mix is crafted with intentional flow because vibes matter.
      </p>
    </motion.div>
    ...
    >
      ...
      <div className="flex space-x-4">
        <motion.a 
          ...
          rel="noopener noreferrer"
          aria-label="WhatsApp"
          className={`p-3 rounded-full ${isDarkMode ? 'bg-gray-800 hover:bg-gray-700' : 'bg-white hover:bg-gray-200'} shadow-md`}
        >
    ...
    >
      <h3 className="text-lg font-semibold mb-3">Legal Notice</h3>
      <p className={`text-xs leading-relaxed ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
        © 2025 Golden Archive. All rights reserved. The music featured in these mixes remains the
        property of its original creators. These mixes represent original curation and arrangement
        by Golden Archive and are offered for entertainment purposes only. By purchasing, you're 
        supporting the creative effort behind the mixing not licensing the individual tracks.
      </p>
    </motion.div>
  </div>
</footer>
